...
import { updateActionPlan, getPenerimaManfaatByDesa, getActionPlanById } from '@/lib/actions/action-plan'
...
export default function EditActionPlanPage() {
    ...
    if (id) {
      getActionPlanById(id).then((data: any) => {
        if (data) {
          ...
          setPilihanProgram(data.pilihan_program || '')
          setTahunAktivasi(data.tahun_aktivasi)
          setKeteranganSe(data.keterangan_se || '')
          setJenisSampah(data.jenis_sampah || '')
          setKapasitasPengelolaan(data.kapasitas_pengelolaan || '')
          setJumlahPengajar(data.jumlah_pengajar || '')
          setJumlahPp(data.jumlah_pp || '')
          setFokusProgram(data.fokus_program || '')
          setCakupanProgram(data.cakupan_program || '')
          setSasaranProgram(data.sasaran_program || '')
          setLegalitas(data.legalitas || '')
          
          if (data.activities && data.activities.length > 0) {
            if (data.kategori_program === 'EKONOMI') {
              const rabMap = new Map()
              const distMap = new Map()

              data.activities.forEach((act: any) => {
                const u = act.uraian_kebutuhan
                if (!rabMap.has(u)) {
                  rabMap.set(u, {
                    id: Date.now() + Math.random(),
                    bulan_implementasi: '', // Ditangani oleh Timeline
                    uraian_kebutuhan: u,
                    nominal_rencana: 0,
                    jumlah_unit: act.jumlah_unit || '',
                    frekuensi: act.frekuensi || '',
                    harga_satuan: act.harga_satuan || ''
                  })
                }
                const rabRow = rabMap.get(u)
                
                if (!distMap.has(rabRow.id)) distMap.set(rabRow.id, {})
                const monthDist = distMap.get(rabRow.id)
                
                rabRow.nominal_rencana += Number(act.nominal_rencana || 0)
                
                const month = Number(act.bulan_implementasi)
                if (month >= 1 && month <= 12) {
                  monthDist[month] = act.nominal_rencana?.toString() || ''
                }
                
                if (act.jumlah_unit) rabRow.jumlah_unit = act.jumlah_unit
                if (act.frekuensi) rabRow.frekuensi = act.frekuensi
                if (act.harga_satuan) rabRow.harga_satuan = act.harga_satuan
              })
              
              Array.from(rabMap.values()).forEach((rabRow: any) => {
                 rabRow.nominal_rencana = (Number(rabRow.jumlah_unit)||0) * (Number(rabRow.frekuensi)||0) * (Number(rabRow.harga_satuan)||0)
              })

              setActivities(Array.from(rabMap.values()))
              setTimelineDistributions(Object.fromEntries(distMap))
            } else {
          ...
          if (data.beneficiaries && data.beneficiaries.length > 0) {
            setSelectedPms(data.beneficiaries.map((b: any) => ({
              pm_id: b.pm_id,
              nama: b.pm_name,
              penghasilan_awal: b.penghasilan_awal || '',
              tanggungan: b.tanggungan || '',
              status_gk: b.status_gk || '',
              status_hk: b.status_hk || '',
              selisih_gk: b.selisih_gk || '',
              nib_halal: b.nib_halal || ''
            })))
          }
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updateActionPlan(id, payloadInduk, payloadActivities, payloadBeneficiaries)
    
    setLoading(false)
    if (result.success) {
      router.push(`/dashboard/action-plan/${id}`)
    } else {
  ...
  if (loadingData) {
    return <div className="p-12 text-center text-slate-500">Memuat data form edit...</div>
  }
  ...
  return (
    <div className="p-4 lg:p-6 max-w-screen-xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Edit Action Plan</h1>
          <p className="text-slate-500 text-sm mt-1">Perbaiki Rencana Anggaran Biaya (RAB) Program</p>
        </div>
      </div>
