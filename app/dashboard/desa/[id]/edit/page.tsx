...
export default function EditDesaBinaanPage() {
  ...
  const desaBinaanId = idStr ? parseInt(idStr) : 0
    ...
    async function initData() {
      try {
        ...
        if (desaBinaanId) {
          ...
          if (desaData) {
            setFormData({
              provinsi_id: desaData.provinsi_id?.toString() || '',
              ...
              potensi_desa: desaData.potensi_desa || '',
              status_aktif: desaData.status_aktif !== false
            })

            // Populate child dropdowns based on existing data
            if (desaData.provinsi_id) {
              const kotas = await getKotaKabupaten(desaData.provinsi_id)
              setKotaOptions(kotas)
            }
            if (desaData.kota_id) {
              const kecamatans = await getKecamatan(desaData.kota_id)
              setKecamatanOptions(kecamatans)
            }
            if (desaData.kecamatan_id) {
              const desas = await getDesaConfig(desaData.kecamatan_id)
              setDesaConfigOptions(desas)
            }
          }
        }
      } catch (err) {
        console.error('Failed to init data:', err)
        alert('Gagal memuat data desa binaan.')
  ...
  const handleDropdownChange = async (name: string, value: string) => {
    ...
    if (name === 'provinsi_id') {
      try {
        const kotas = await getKotaKabupaten(numVal)
        setKotaOptions(kotas)
        setKecamatanOptions([])
      ...
      try {
        const kecamatans = await getKecamatan(numVal)
        setKecamatanOptions(kecamatans)
        setDesaConfigOptions([])
      ...
      try {
        const desas = await getDesaConfig(numVal)
        setDesaConfigOptions(desas)
  ...
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (type === 'checkbox') {
      ...
      if (['provinsi_id', 'kota_id', 'kecamatan_id'].includes(name)) {
  ...
  // Avoid running the auto-search on the initial load if coordinates exist
  useEffect(() => {
    if (!initialLoading) {
      setTimeout(() => setFirstLoadDone(true), 1500)
    }
  }, [initialLoading])

  // Construct auto search query whenever a region is fully selected
  useEffect(() => {
    if (firstLoadDone && formData.desa_id && formData.kecamatan_id && formData.kota_id && formData.provinsi_id) {
  ...
  }, [formData.desa_id, formData.kecamatan_id, formData.kota_id, formData.provinsi_id, desaConfigOptions, kecamatanOptions, kotaOptions, provinsiOptions, firstLoadDone])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      await updateDesaBinaan(desaBinaanId, {
        ...formData,
      ...
      alert('Perubahan Desa Binaan disimpan!')
      router.push('/dashboard/desa')
    } catch (error: any) {
      console.error(error)
      alert('Gagal menyimpan desa binaan: ' + (error.message || 'Unknown error'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-end mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Edit Desa Binaan</h1>
          <p className="text-slate-500 text-sm mt-1">Sesuaikan kembali wilayah atau relawan penanggung jawab.</p>
        </div>
      </div>
