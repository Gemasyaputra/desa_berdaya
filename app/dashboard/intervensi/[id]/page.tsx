      ...
      <div className="sticky top-0 z-10 bg-white border-b border-slate-200 shadow-sm px-4 lg:px-8 py-4 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {headerData.action_plan_id && (
              <Button 
                variant="outline" 
                size="sm"
                className="text-indigo-600 border-indigo-200 hover:bg-indigo-50 rounded-lg h-7 px-3 text-xs shadow-sm bg-white"
                onClick={() => router.push(`/dashboard/action-plan/${headerData.action_plan_id}`)}
              >
                <FileText className="w-3 h-3 mr-1" /> Lihat Action Plan
              </Button>
            )}
          </div>
        </div>
