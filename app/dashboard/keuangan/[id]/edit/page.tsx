...
import { getDesaBerdayaOptions, updateLaporanKeuangan, getLaporanKeuanganById, uploadBukti } from '../../actions'

export default function EditLaporanPage() {
  const router = useRouter()
  const params = useParams()
  const idStr = params?.id as string
  const laporanId = idStr ? parseInt(idStr) : 0
  ...
  useEffect(() => {
    async function initData() {
      try {
        ...
        if (laporanId) {
          const laporanInfo = await getLaporanKeuanganById(laporanId)
          if (laporanInfo) {
            setFormData({
              desa_berdaya_id: laporanInfo.desa_berdaya_id || 0,
              judul_kegiatan: laporanInfo.judul_kegiatan || '',
              deskripsi: laporanInfo.deskripsi || '',
              total_realisasi: laporanInfo.total_realisasi ? new Intl.NumberFormat('id-ID').format(laporanInfo.total_realisasi) : '',
              bukti_url: laporanInfo.bukti_url || '',
              jenis_kegiatan: laporanInfo.jenis_kegiatan || 'EKONOMI',
            })
            const matchingDesa = options.find((d: any) => d.id === (laporanInfo.desa_berdaya_id || 0))
            if (matchingDesa) {
              setSelectedDesa(matchingDesa)
        ...
        alert("Gagal memuat data Laporan Kegiatan")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true)
      await updateLaporanKeuangan(laporanId, { 
        ...formData, 
        total_realisasi: realisasiNum 
      })
      alert('Laporan Kegiatan & Keuangan berhasil diperbarui!')
      router.push('/dashboard/keuangan')
    } catch (error) {
      console.error(error)
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-end mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Edit Laporan Keuangan</h1>
          <p className="text-slate-500 text-sm mt-1">Ubah atau perbarui data realisasi kegiatan di Desa Binaan.</p>
        </div>
      </div>
        ...
        <div className="col-span-1 lg:col-span-8">
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-5">
            ...
            <div className="pt-6 border-t flex justify-end gap-3">
              <Button type="button" variant="outline" onClick={() => router.push('/dashboard/keuangan')}>
                Batal
              </Button>
