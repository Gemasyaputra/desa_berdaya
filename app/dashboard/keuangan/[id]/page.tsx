  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const data = await getLaporanKeuanganById(id)
        if (data) {
          setLaporan(data)
        ...
        <div className="xl:col-span-2 space-y-8">
          <Card className="bg-white border-none shadow-xl shadow-slate-200/50 rounded-[2rem] p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
              ...
              <Card className="bg-amber-50 border-amber-100 rounded-3xl p-6 h-fit">
                <p className="text-amber-600 text-[10px] font-black uppercase tracking-widest mb-1">Total Realisasi</p>
                <p className="text-3xl font-black text-amber-900 font-mono tracking-tighter">
                  {formatRupiah(laporan.total_realisasi)}
                </p>
                {laporan.is_terdokumentasi && (
                  <div className="mt-4 inline-flex items-center gap-1.5 px-3 py-1 bg-green-100 text-green-700 rounded-full text-[10px] font-black uppercase tracking-tight">
