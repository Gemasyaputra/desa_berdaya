...
import { updatePenerimaManfaat, getDesaBerdayaOptions, getPenerimaManfaatById } from '../../actions'

export default function EditPMPage() {
  const router = useRouter()
  const params = useParams()
  const idStr = params?.id as string
  const pmId = idStr ? parseInt(idStr) : 0
  ...
  useEffect(() => {
    async function initData() {
      try {
        ...
        if (pmId) {
          const pmInfo = await getPenerimaManfaatById(pmId)
          if (pmInfo) {
            setFormData({
              desa_berdaya_id: pmInfo.desa_berdaya_id || 0,
              nik: pmInfo.nik || '',
              nama: pmInfo.nama || '',
              tempat_lahir: pmInfo.tempat_lahir || '',
              tanggal_lahir: pmInfo.tanggal_lahir ? new Date(pmInfo.tanggal_lahir).toISOString().split('T')[0] : '',
              jenis_kelamin: pmInfo.jenis_kelamin || '',
              golongan_darah: pmInfo.golongan_darah || '',
              alamat: pmInfo.alamat || '',
              rt_rw: pmInfo.rt_rw || '',
              kel_desa: pmInfo.kel_desa || '',
              kecamatan: pmInfo.kecamatan || '',
              agama: pmInfo.agama || '',
              status_perkawinan: pmInfo.status_perkawinan || '',
              pekerjaan: pmInfo.pekerjaan || '',
              kewarganegaraan: pmInfo.kewarganegaraan || '',
              kategori_pm: pmInfo.kategori_pm || 'EKONOMI',
              foto_ktp_url: pmInfo.foto_ktp_url || '',
            })
            if (pmInfo.foto_ktp_url) {
              setPreviewImage(pmInfo.foto_ktp_url)
            }
          }
        }
      } catch (error) {
        console.error("Gagal load PM", error)
        alert("Gagal memuat data Penerima Manfaat")
  ...
  return (
    <div className="p-4 lg:p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Edit Penerima Manfaat</h1>
        <p className="text-slate-500 text-sm mt-1">Perbarui biodata peserta atau unggah ulang dokumen KTP jika ada kesalahan.</p>
      </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
              ...
              <div className={`relative border-2 border-dashed rounded-xl overflow-hidden aspect-[1.6/1] flex flex-col items-center justify-center transition-all bg-slate-50 hover:bg-slate-100 ${scanStatus === 'scanning' ? 'border-red-400 bg-red-50' : 'border-slate-300'}`}>
                ...
                {previewImage ? (
                  ...
                  <div className="text-center p-6">
                    ...
                    <p className="text-sm font-medium text-slate-600">Pilih Foto KTP Baru</p>
                    <p className="text-xs text-slate-400 mt-1">Tap untuk upload ulang</p>
                  </div>
                )}
              </div>
            </label>
            ...
            {scanStatus === 'success' && (
              <div className="mt-4 bg-green-50 border border-green-200 text-green-700 text-xs p-3 rounded-lg flex items-start gap-2">
                ...
                <p>Data berhasil diekstrak! Silakan periksa kembali perubahannya di form.</p>
              </div>
            )}
          </div>
        </div>
