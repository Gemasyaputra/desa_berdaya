...
const jobs = ["Wiraswasta", "PNS", "Karyawan Swasta", "Petani", "Buruh", "Ibu Rumah Tangga", "Pelajar/Mahasiswa", "Guru", "Pedagang", "Nelayan"];
...
async function updateData() {
  try {
    const res = await pool.query("SELECT id, jenis_kelamin FROM penerima_manfaat WHERE nama LIKE 'Anggota Kesehatan %'");
    console.log(`Found ${res.rows.length} records to update.`);

    for (const row of res.rows) {
      const isLaki = !row.jenis_kelamin || row.jenis_kelamin.toUpperCase() === 'LAKI-LAKI' || Math.random() > 0.5;
      ...
      await pool.query(`
        ...
        SET 
          ...
          kewarganegaraan = 'WNI'
        WHERE id = $11
      `, [fullName, genderStr, tmpLahir, tglLahir, goldar, alamat, rtrw, agama, statusP, pek, row.id]);
    }
